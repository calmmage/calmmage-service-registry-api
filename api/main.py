...
from api.db import (
    upsert_service, get_all_services,
    get_service, get_state_transitions,
    mark_transition_alerted,
    store_heartbeat, get_all_services_status
)
from api.models import (
    Service, ServiceType, ServiceStatus, StateTransition,
    HeartbeatRequest, ServicesStatusResponse
)
...
async def get_service_history(
    ...
    try:
        return await get_state_transitions(
            service_key=service_key,
            limit=limit,
            only_not_alerted=only_not_alerted
        )
    except Exception as e:
...
@app.get("/services/history", response_model=List[StateTransition])
...
@app.post("/services/history/{transition_id}/mark-alerted")
async def mark_alerted(transition_id: str) -> dict:
    """Mark a state transition as alerted"""
    try:
        await mark_transition_alerted(transition_id)
        return {"status": "ok"}
    except Exception as e:
        logger.exception("Failed to mark transition as alerted")
        raise HTTPException(status_code=500, detail=str(e))
